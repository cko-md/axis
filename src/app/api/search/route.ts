...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const [tasksRes, eventsRes, holdingsRes, notesRes] = await Promise.allSettled([
    ...
    supabase.from('schedule_events').select('title, start_at, end_at').eq('user_id', user.id).gte('start_at', new Date().toISOString()).limit(8),
  ...
  if (events.length) ctxParts.push(`Upcoming events: ${events.map((e: { title: string; start_at: string }) => `${e.title} (${new Date(e.start_at).toLocaleDateString()})`).join(', ')}`);
