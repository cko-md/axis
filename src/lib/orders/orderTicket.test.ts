...
describe("buildOrderTicket", () => {
  it("builds a valid market buy with a cent-exact notional", () => {
    const r = buildOrderTicket({ symbol: "aapl", side: "buy", quantity: 10, referencePrice: 212.34, currency: "USD" });
  ...
  it("supports fractional shares", () => {
    const r = buildOrderTicket({ symbol: "MSFT", side: "buy", quantity: 2.5, referencePrice: 400, currency: "USD" });
  ...
  it("requires a positive limit price for a limit order and uses it for notional", () => {
    const bad = buildOrderTicket({ symbol: "NVDA", side: "sell", quantity: 5, type: "limit", referencePrice: 120, currency: "USD" });
    expect(bad.ok).toBe(false);
    const good = buildOrderTicket({ symbol: "NVDA", side: "sell", quantity: 5, type: "limit", limitPrice: 130, referencePrice: 120, currency: "USD" });
  ...
  it("describes a ticket in one line", () => {
    const r = buildOrderTicket({ symbol: "AAPL", side: "buy", quantity: 10, referencePrice: 212, currency: "USD" });
