...
- **Phase 8 — started early** (jumped here per user; the highest-value undone track). Merged to `main`:
  ...
  - **Open Phase-8 work:** AI-2/3/4 (large — typed registry + migrate call sites; do highest-privacy first: triage/route/regimen/financeNarrator), PROD-4 (e2e smoke), PROD-2 (generated Supabase types — large cross-cutting), Leaked Password Protection toggle (Supabase dashboard, no migration). CTRL-*/NAV-* mostly satisfied by prior work (DISP-3 retired /console+/signals; DISP-4 palette coverage).
- **Come back to:** Phase 6 remaining spot-checks (FUND-1 shared cache, PEOPLE-2 import UX — verify-first), Phase 7 lab modules (Vitality/Library/Atelier/Vault/Supper Club — vivid color is legitimate, token-only guards don't apply).
- `main` is current: Phases 0–4 complete, 5 substantial, 6 theme/Debrief fixes, 8 PROD+AI-1 groundwork. Two live migrations this session (`calendar_event_cache`, `key_result_progress`); `profiles_bio` backfilled as tracked-only (already in prod).
