...
- **Come back to:** **FUND-1** (verified genuinely undone, L-complexity) — no `useFund`/FundProvider/FundContext; ~10 fund subroute modules each fetch holdings/quotes independently and there's a shared `src/app/fund/layout.tsx` where a provider could hydrate once. Needs a shared store to dedupe cross-subroute fetches. PEOPLE-2 verified done earlier (import/match + merge flow exists). Remaining big Phase-8: PROD-4 (e2e smoke), full PROD-2 wiring, AI-2 typed-client adoption for the remaining raw calls.
