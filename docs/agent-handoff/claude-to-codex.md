...
- **Next: finish Phase 6 spot-checks, then Phase 7** (lab modules — Vitality/Library/Atelier/Vault/Supper Club; these legitimately use vivid color, so a token-only theme guard does NOT apply), then Phase 8 (Control Room/Nav/AI/prod).
  - `main` is current with Phases 0–5 substantial + Phase-6 theme/Debrief fixes.
