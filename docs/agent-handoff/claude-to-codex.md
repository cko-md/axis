...
- **Come back to:** Phase 6 remaining spot-checks (FUND-1 shared cache, PEOPLE-2 import UX — verify-first), remaining Phase 7 lab modules (Library/Atelier/Supper Club — honesty/complete-workflow spot-check; vivid color is legitimate so token-only guards don't apply).
