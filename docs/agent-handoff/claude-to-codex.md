...
- **Codex rescue-branch reconciliation (done):** the concurrent `codex-rescue` agents produced ~26 branches. **20 clean hardening fixes cherry-picked to `main`** (fund/calendar/mail/people/briefing/signals error-surfacing); `3dd5ab3 fix(fund): bound finance narrator context` also landed (`e12af7c`). 2 superseded branches **deleted** (`rescue-console-triage-typed-ai-action`, `rescue-password-pwned-padding-parser` — equivalent already on main). **5 branches PRESERVED (not deleted) — they carry unique unlanded value but conflict with this session's mainline refactors (AI registry / FUND-1 / PROD-2 typed client):**
  - `codex/rescue-notes-study-aids-typed-ai` — types flashcards/quiz/mindmap/study-summary AI calls (I migrated meetingSummary/route/summarize/rewrite/title but NOT the study aids). Conflicts on `actions.ts` + `NotesModule`.
  - `codex/rescue-mail-calendar-status-errors` — surfaces account-status failures across 13 mail/calendar files. Conflicts on `mail/composio`, `mail/tokens` (my encryption guard), status routes.
  - `codex/rescue-fund-order-safety` — honest order-capture states in `FundOrderTicket`/`FundInvestingModule` (conflicts with FUND-1 refactor).
  - `codex/rescue-calendar-sync-discovery-errors` — sync-account discovery failures in `calendar/sync` (+ a route test); conflicts with my PROD-2 cast there.
  - `codex/rescue-vitality-capture-typed-ai` — may be redundant with my vitality meal-parse/meditation migration; verify before landing.
  - **To land:** `git checkout main && git cherry-pick <sha>`, resolve the conflict (keep both the mainline refactor AND the branch's failure-surfacing/typed-migration intent), gate, commit. Do these one at a time.
- **Rescue-branch reconciliation — 5 preserved branches now RESOLVED (done):** all landed one at a time with conflicts resolved + full gate (tsc/lint/334 tests/prod build green):
  - `codex/rescue-notes-study-aids-typed-ai` → `8608807`. Added 4 typed registry actions (flashcards/quiz/mindmap/studySummary) + routed NotesModule study-aid generation through `buildAiRequestBody`. Test conflict was purely additive (kept both assertion blocks).
  - `codex/rescue-phase-6-fund-order-safety` → `f56bcdb`. Kept the honest order-capture states + confirm dialog; dropped a duplicate "Holding removed" toast (main already toasts after `load()`).
  - `codex/rescue-calendar-sync-discovery-errors` → `2c3f6c9`. Landed the new `route.test.ts`; kept main's *superior* connection-error responses (specific messages + `code:"connection_lookup_failed"` + 502 for composio) and updated the two incoming test assertions to match main rather than downgrade the route.
  - `codex/rescue-mail-calendar-status-errors` → `756d647` (12 files). Only `calendar/external/route.ts` conflicted. Adopted the rescue branch's `{events:[],partial:true,errors:[...]}` 503 shape for the composio-discovery catch because **`ScheduleModule` reads `r.json()` without checking `r.ok` and only consumes `events/partial/errors`** — main's prior `{error,code}` 500/502 shape was a silent UI failure. Also removed a **dead duplicate `if(connectionsError)` block** left on main from an earlier landing. My `mail/tokens.ts` encryption guard survived the auto-merge.
  - `codex/rescue-vitality-capture-typed-ai` → **superseded, not landed.** Main already routes meditation via `callAiAction("capture")` and meal-parse via the dedicated `callAiAction("mealParse")` action — strictly better than the branch's `buildAiRequestBody("capture")` (still the buggy meal-parse path). Local branch delete attempted; remote branch left as-is (harmless; value already on main). Safe to delete when convenient.
