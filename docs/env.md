...
| Direct-provider v1 migration | `DIRECT_PROVIDER_COOKIE_V1_ACCEPT_UNTIL` (strict ISO timestamp); optionally retain `SPOTIFY_CLIENT_SECRET_PREVIOUS` / `STRAVA_CLIENT_SECRET_PREVIOUS` only until that deadline |
...
- Direct Spotify/Strava credentials and OAuth state use `DIRECT_PROVIDER_COOKIE_SECRET` as the stable AXIS-owned v2 signing key. Keep the prior AXIS key in `DIRECT_PROVIDER_COOKIE_SECRET_PREVIOUS` during a planned rotation, confirm hosted traffic has re-signed credentials, then retire it. Provider-secret-derived v1 cookies are accepted only before the strict `DIRECT_PROVIDER_COOKIE_V1_ACCEPT_UNTIL` deadline and every authenticated read writes v2 while removing same-subject legacy copies. Set the deadline to cover the maximum pre-deploy cookie lifetime (90 days); if a provider secret must rotate inside that window, place only its prior value in the matching `*_CLIENT_SECRET_PREVIOUS` variable and remove it with the v1 deadline. Missing or expired v1 configuration fails closed.
- Refresh admission keys are derived from the authenticated credential generation and high-entropy refresh token, not an AXIS signing key. This keeps concurrent refresh coordination stable across rolling signing-key deployments. A consumed-generation claim is written before provider traffic; Redis cleanup after provider traffic is best-effort and observable, so cleanup failure cannot discard a successful rotated credential.
